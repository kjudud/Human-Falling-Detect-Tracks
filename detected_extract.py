        ...
        if camera == '1':
            if not first_detect: 
                for det_idx in range(detected.shape[0]):
                    if (first_detect == False) and (detected[det_idx][3] > 260) and \
                        ((detected[det_idx][2] - detected[det_idx][0] + detected[det_idx][3] - detected[det_idx][1]) >= 180):
                ...
                for det_idx in range(detected.shape[0]):
                    if first_detect:
                        ...
                        if detected[det_idx][4] != torch.tensor([0.5],dtype=torch.float32):
                            iou *= 1.2
                        if (iou > 0.3) and (detected[det_idx][1]>=130 and detected[det_idx][2]<=350 and detected[det_idx][3] > 260) and \
                    ...
                    if (not first_detect) and (detected[det_idx][1]>=130 and detected[det_idx][2]<=350)  (detected[det_idx][3] > 260) and \
                        ((detected[det_idx][2] - detected[det_idx][0] + detected[det_idx][3] - detected[det_idx][1]) >= 150):
                        tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                        torch.save(detected[det_idx], tensorpath)
                        tensor_save_state = True
                ...
                for det_idx in range(detected.shape[0]):
                    if first_detect:
                        ...
                        if detected[det_idx][4] != torch.tensor([0.5],dtype=torch.float32):
                            ...
                            iou *= 0.7
                        if (iou > 0.2) and (detected[det_idx][0]>=80 and detected[det_idx][2]<=280) and (detected[det_idx][3] >= 210) \
                            and (detected[det_idx][2]-detected[det_idx][0]+detected[det_idx][3]-detected[det_idx][1]>100):
                            if iou > min_iou:
                                min_iou = iou
                                tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                                torch.save(detected[det_idx], tensorpath)
                                tensor_save_state = True
                    if (not first_detect) and (detected[det_idx][0]>=80 and detected[det_idx][2]<=280) and (detected[det_idx][3] >= 210) \
                        and (detected[det_idx][2]-detected[det_idx][0]+detected[det_idx][3]-detected[det_idx][1]>60):
                        tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                        torch.save(detected[det_idx], tensorpath)
                        tensor_save_state = True
                        first_detect = True     
