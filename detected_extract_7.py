...
if __name__ == '__main__':
    ...
    while cam.grabbed():
        ...
        if camera == '1':
            if not first_detect: 
                for det_idx in range(detected.shape[0]):
                    if (first_detect == False) and (detected[det_idx][2]<=270) and (detected[det_idx][3] > 260) and \
        ...
        #camera 2
            ...
            if not first_detect: 
                for det_idx in range(detected.shape[0]):
                    if (first_detect == False) and (detected[det_idx][0]>=0 and detected[det_idx][2]<=230) and \
                ...
                for det_idx in range(detected.shape[0]):
                    if first_detect:
                        ...
                        if detected[det_idx][4] != torch.tensor([0.5],dtype=torch.float32):
                            iou *= 1.2
                        if (iou > 0.3) and (detected[det_idx][2]<=270) and (detected[det_idx][3] > 260) and \
                        ((detected[det_idx][2] - detected[det_idx][0] + detected[det_idx][3] - detected[det_idx][1]) >= 150):
                            if iou > min_iou:
                                min_iou = iou
                                tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                    ...
                    if (not first_detect) and (detected[det_idx][2]<=270) and (detected[det_idx][3] > 260) and \
                        ((detected[det_idx][2] - detected[det_idx][0] + detected[det_idx][3] - detected[det_idx][1]) >= 150):
                        tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                        torch.save(detected[det_idx], tensorpath)
                        tensor_save_state = True
                ...
                for det_idx in range(detected.shape[0]):
                    if first_detect:
                        ...
                        if detected[det_idx][4] != torch.tensor([0.5],dtype=torch.float32):
                            ...
                            iou *= 0.7
                        if (iou > 0.2) and (detected[det_idx][0]>=0 and detected[det_idx][2]<=230) and \
                            (detected[det_idx][2]-detected[det_idx][0]+detected[det_idx][3]-detected[det_idx][1]>150) and (detected[det_idx][3] >= 220):
                            if iou > min_iou:
                                min_iou = iou
                                tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                                torch.save(detected[det_idx], tensorpath)
                                tensor_save_state = True
                    if (not first_detect) and (detected[det_idx][0]>=0 and detected[det_idx][2]<=230) and \
                        (detected[det_idx][2]-detected[det_idx][0]+detected[det_idx][3]-detected[det_idx][1]>150) and (detected[det_idx][3] >= 220):
                            tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                            torch.save(detected[det_idx], tensorpath)
                            tensor_save_state = True
