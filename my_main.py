...
if __name__ == '__main__':
    ...
    while cam.grabbed():
        ...
        #my_getframe파일로 대체
        #image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        # frame_path = 'HAR-UP-frame/test/frame%d.png' % r
        # cv2.imwrite(frame_path, image)

        # Detect humans bbox in the frame with detector model.
        #내가 주석으로 바꾼 라인
        #detected = detect_model.detect(frame, need_resize=False, expand_bb=10)
        # file = open("./detected_camera1.txt", 'a')
        # file.write(str(detected)+str(r)+'\n')
        # file.close()
        #detected값 이용할때
        ...
        #내가 추가한 라인
        #detected 텐서저장라인
        # tensorpath = './out/detection/HAR-UP-1-1/result%d.t' % r
        # torch.save(detected, tensorpath)
        # file = open("./detectedresult.txt", 'a')
        # file.write(str(detected)+'\n')
        # file.close()
        #detected 텐서저장라인
        # tensorpath = f'./out/detection/HAR-UP-{subject}-1-{trial}-{camera}/result{r}.t'
        # torch.save(detected, tensorpath)
        ...
        #         torch.save(detected[0], tensorpath)

        # if camera == '1':
        #     for track in tracker.tracks:
        #         det = torch.tensor([track.to_tlbr().tolist() + [0.5, 1.0, 0.0]], dtype=torch.float32)
        #         detected = torch.cat([detected, det], dim=0) if detected is not None else det
                # file = open("./result_txt/detected_track-1-2.txt", 'a')
                # file.write(str(detected)+str(r)+'\n')
                # file.close()
                ...
                #         if (detected[det_idx][1]>=130 and detected[det_idx][2]<=350) and ((detected[det_idx][2] - detected[det_idx][0] + detected[det_idx][3] - detected[det_idx][1]) >= 180):
                            #tensorpath = f'./out/detection/HAR-UP-{subject}-1-{trial}-{camera}/result{r}.t'
                            # tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                            #torch.save(detected[det_idx], tensorpath)
                            # file = open("result_txt/detected_inroi1131.txt", 'a')
                            # file.write(str(detected[det_idx])+str(r)+'\n')
                            # file.close()
                            # break
                # camera 1
                # if (first_detect == False) and (detected[det_idx][1]>=130 and detected[det_idx][2]<=350) and ((detected[det_idx][2] - detected[det_idx][0] + detected[det_idx][3] - detected[det_idx][1]) >= 180):
                    #tensorpath = f'./out/detection/HAR-UP-{subject}-1-{trial}-{camera}/result{r}.t'
                    # tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                    #torch.save(detected[0], tensorpath)
                    # file = open("result_txt/detected_inroi1131.txt", 'a')
                    # file.write(str(detected[0])+str(r)+'\n')
                    # file.close()
                    # first_detect = True
        ...
        #         for det_idx in range(detected.shape[0]):
        #             if detected[det_idx][4] == torch.tensor([0.5], dtype=torch.float32):
        #                 if (detected[det_idx][0]>=80 and detected[det_idx][2]<=280) and (detected[det_idx][3] >= 190):
                            #tensorpath = f'./out/detection/HAR-UP-{subject}-1-{trial}-{camera}/result{r}.t'
                            # tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                            # torch.save(detected[det_idx], tensorpath)
                            # file = open("result_txt/detected_inroi2.txt", 'a')
                            # file.write(str(detected[det_idx])+str(r)+'\n')
                            # file.close()
                            # break
                # camera 2 
                # if (first_detect == False) and (detected[det_idx][0]>=80 and detected[det_idx][2]<=280) and (detected[det_idx][3] >= 190):
                #     tensorpath = './out/detection/HAR-UP-1-2/result%d.t' % r
                #     tensorpath = os.path.join(det_tensor_save_dir,f'result{r}.t')
                #     torch.save(detected[0], tensorpath)
                    # file = open("result_txt/detected_inroi2.txt", 'a')
                    # file.write(str(detected[0])+str(r)+'\n')
                    # file.close()
                    # first_detect = True
        # file = open("./result_txt/detected_track_1131.txt", 'a')
        # file.write(str(detected)+str(r)+'\n')
        # file.close()     
            ...
            for i, track in enumerate(tracker.tracks):
                ...
                if len(track.keypoints_list) == 30:
                    ...
                    if len(action_list[i]) == action_win:
                        ...
                        #     file = open(f'./acc-avtivity-{activity}.txt', 'a')
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        # if f>=30 and f<=50 :
            # rame.shape-->(768,768,3)
            # 5-1-3-1 자를때
            #save_frame = frame[200:738,:598]
            # 7-9-2-1 자를때
            # save_frame = frame[200:738,50:648]
            # cv2.imwrite(f'results/HAR-UP-8-1-1-1-action_{f}.png', frame)
